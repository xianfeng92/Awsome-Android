...
线程池的空闲线程都有超时机制, CachedThreadPool 的任务队列其实相当于一个空集合, 这就导致任何任务都会立即被执行. SynchronousQueue 是一个非常特殊的队列，在很多情况下可以把它简单理解为一个无法存储的元素队列. 这类线程池比较适合执行大量的耗时较少的任务, 当整个线程池属于闲置状态的时候, 线程池中的线程都会超时而被停止，这个时候 CachedThreadPool 之中实际上是没有任何线程, 这个时候它几乎是不占用任何资源的。
